...
La teoría del error total de encuesta es un marco conceptual que intenta describir los errores estadísticos de una encuesta incorporando distintos tipos de errores que pueden suceder a lo largo de todo el proceso de elaboración de una encuesta, no solo del muestreo ([Grover y Lyberg, 2010](https://academic.oup.com/poq/article-abstract/74/5/849/1817502)). A diferencia del error muestral en muestras probabilísticas muchos de las otras fuentes de error de una encuesta no pueden ser estimados matemáticamente, aunque si existe mucha literatura sobre como reducirlos. 

Algunos de los errores no muestrales a tener en cuenta son:

<ol>
  <li>**Sesgo de no respuesta:** este tipo de error puede ocurrir cuando los encuestados seleccionados en la muestra no pueden o se niegan a ser contactados. Este es uno de los problemas más frecuentes y relevantes de la investigación en opinión pública, con una tendencia al aumento de la [tasa de respuesta](https://www.aapor.org/Education-Resources/For-Researchers/Poll-Survey-FAQ/Response-Rates-An-Overview.aspx). Este error puede sesgar la muestra si las personas que se niegan o no pueden participar difieren sistemáticamente de las que si participan. </li>
  <li> **Error de cobertura:** ocurre cuando hay un desfasaje entre la población objetivo y la población del marco muestral. Ejemplo: cuando se utilizan encuestas telefónicas y las personas que no tienen teléfono son de interés para la investigación (nunca podrían ser seleccionadas para la encuesta) </li>
  <li>**Errores en las respuestas:** los encuestados pueden no responder de forma sincera si no están lo suficientemente motivados como para hacerlo, o las preguntas pueden no ser lo suficientemente claras. A su vez, los encuestadores pueden cometer inducir error sistemático al encuestar ([Weisberg, 2018](https://www.degruyter.com/document/doi/10.7208/9780226891293/html)). </li>
</ol>
...
En la sección anterior se presentan actitudes hacia instituciones políticas. Esos resultados pueden interpretarse como un análisis de coyuntura por parte de los ciudadanos, pero si la disatisfacción se generaliza hacia el régimen político y sus valores, puede causar una crisis democrática ([Dalton, 2013](https://books.google.es/books?hl=es&lr=&id=Sl4XBAAAQBAJ&oi=fnd&pg=PP1&dq=russel+dalton+2013+citizens&ots=jbkuDRXY4t&sig=aTiUMSL7h7fnTp4h2_-z5P8GEd0#v=onepage&q=russel%20dalton%202013%20citizens&f=false)). Para medir opiniones sobre el régimen democrático -o apoyo hacia la democracia- una de las preguntas que se utiliza es si la democracia es el mejor sistema del gobierno. Este tipo de preguntas son muy frecuentes en proyectos de opinión pública internacionales. La [Encuesta Mundial de Valores](https://www.worldvaluessurvey.org/WVSContents.jsp) -proyecto que realiza encuestas en más de 120 países y en todos los continentes- sugiere que el apoyo a la democracia es mayoritario en casi todos los países participantes del estudio ([Thomassen, 2007](https://research.utwente.nl/en/publications/democratic-values)). Es importante mencionar que lo que se entiende por democracia puede variar según el país, por lo que se debe ser cauteloso a la hora de comparar los resultados de estas preguntas en distintos países ([Joignant et al., 2017](https://link.springer.com/chapter/10.1057/978-1-137-59955-1_1)).
...
demo_apoyo_p <- ggplot(demo_apoyo,
       aes(x = year, y = value, 
           color = stringr::str_wrap(cat, 35), 
           fill = stringr::str_wrap(cat, 35))) +
  geom_line(size=1.25) +
  geom_point(size=3, shape=21, fill="white") +
  theme_minimal() +
  theme(legend.position = "bottom") +
...
demo_churc <- readxl::read_excel("data/latino/demo_serie.xlsx") %>%
  mutate(value = round(value * 100, digits = 0),
         year = as.numeric(year)) %>% 
  filter(pregunta == "churchill")

# Tabla
demo_churc_tabla <- demo_churc %>% 
  select(-pregunta) %>% 
  pivot_wider(names_from = cat,
              values_from = value)

demo_churc_p <- ggplot(demo_churc,
       aes(x = year, y = value, 
           color = cat, fill = cat)) +
  geom_line(size=1.25) +
  geom_point(size=3, shape=21, fill="white") +
...
```{r, echo=FALSE, fig.align="center"}
demo <- readxl::read_excel("data/latino/tabla_demo.xlsx") %>% 
  select(Pais, satisfecho) %>% 
  arrange(desc(satisfecho))

tab <- gt(demo) %>% 
    cols_label(Pais = "País",
               satisfecho = "% satisfecho o muy satisfecho con la democracia") %>% 
    tab_header(title = md("Satisfacción con la democracia en América Latina en 2020")) %>% 
    tab_source_note(source_note = "Fuente: Unidad de Métodos y Acceso a Datos (FCS-UdelaR) en base a datos de Latinobarómetro")
...
# Con bar
(tab_2 <- tab %>%
    gt_plt_bar(column = satisfecho, scaled = TRUE, 
               color = "#2c3e50", scale_type = "number") %>% 
    gt_highlight_rows(rows = 1, fill = "lightblue",
                      bold_target_only = TRUE, target_col = Pais) %>% 
    gt_highlight_rows(rows = 8, fill = "lightgrey",
...
demo_sat <- readxl::read_excel("data/latino/demo_serie.xlsx") %>%
  mutate(value = round(value * 100, digits = 0),
         year = as.numeric(year)) %>% 
  filter(pregunta == "satisfaccion")

# Tabla
demo_sat_tabla <- demo_sat %>% 
  select(-pregunta) %>% 
  pivot_wider(names_from = cat,
              values_from = value)
...
Otro indicador frecuente en encuestas de opinión pública es la percepción sobre el principal problema del país. La percepción sobre el principal problema del país es un indicador para la prominencia o relvancia de cada tema -*issue salience*-. Cuando un tema es importante para el público los candidatos y partidos suelen tomar posición al respecto ([Wlezien y Soroka, 2007](https://scholar.google.es/scholar?hl=es&as_sdt=0%2C5&q=The+Relationship+between+Public+Opinion+and+Policy+oxford+handbook&btnG=)). A su vez, los ciudadanos suelen preocuparse y tener opiniones formadas respecto a estos temas. Aunque es importante destacar que la pregunta sobre el principal problema del país no siempre es el mejor indicador para obtener la saliencia de todos los temas debido a que al preguntar por problemas no surgen temas donde el gobierno haya actuado bien ([Wlezien, 2005](https://www.sciencedirect.com/science/article/pii/S026137940500020X?casa_token=Al5HAqI4XjQAAAAA:_dsDUL4RfBGvSSD1ldWERlHitCqIpiQsy0QQmI5SQq0XKHu77_eI6YkfAFy2ExOJGnn8TNRpKQM)). 
...
Dalton, R. J. (2004). Democratic challenges, democratic choices. Oxford univ. press.Dalton, R. J. (2004). Democratic challenges, democratic choices. Oxford univ. press.
...
Lyberg, L., & Weisberg, H. (2016). Total survey error: a paradigm for survey methodology (pp. 27-42). Thousand Oaks, CA: Sage.
...
Thomassen, J. (2007). Democratic values. In The Oxford handbook of political behaviour (pp. 418-434). Oxford University Press.
...
Traugott, P. D. M. W., & Lavrakas, P. D. P. J. (2016). The voter's guide to election polls. Lulu. com.
...
Weisberg, H. F. (2009). The total survey error approach. In The Total Survey Error Approach. University of Chicago Press.
...
Wlezien, C., & Soroka, S. N. (2007). The relationship between public opinion and policy. In The Oxford handbook of political behavior.
